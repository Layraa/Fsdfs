...
public class FollowNodeExecutor implements NodeExecutor {
    ...
    private static final Map<Integer, Boolean> isWalking = new HashMap<>();
    ...
    public boolean execute(CustomMobEntity entity, BehaviorNode node, BehaviorTreeExecutor executor) {
        ...
        boolean currentlyWalking = isWalking.getOrDefault(entityId, false);
        ...
        if (lastMoveTime.containsKey(entityId)) {
            ...
            if (currentTime - lastMove < duration) {
                ...
                // Время движения истекло - сбрасываем флаг и проверяем позицию
                isWalking.put(entityId, false);
        ...
        // Если уже находимся на нужном расстоянии (с погрешностью), возвращаем успех
